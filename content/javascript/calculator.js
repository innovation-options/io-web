...
function calculateOption() {
  ...
  function createTable(valueTree, iterations) {
  ...
  for (let i = 1; i <= tranchCount; i++) {
    tranchKey = `tranch-${i}`
    compoundOutput += `<p>premium: ${data[tranchKey]['premium']}</p>${createTable(data[tranchKey]['value_tree'], data[tranchKey]['iterations'])}`

  };
  // Render it!
  document.getElementById('premium').innerHTML = `P is: ${compoundOutput}`;
